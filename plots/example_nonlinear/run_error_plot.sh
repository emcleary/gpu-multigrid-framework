#!/bin/env bash
executable=../../build/bin/examples/nonlinear_1d_2nd_order
...
for i in `seq $n_levels_min $n_levels_max`; do
    ...
    # $executable $i $linear $on_cpu >> results_linear_cpu.txt
    # $executable $i $linear $on_gpu >> results_linear_gpu.txt
    # $executable $i $nonlinear_full $on_cpu >> results_nonlinear_full_cpu.txt
    # $executable $i $nonlinear_full $on_gpu >> results_nonlinear_full_gpu.txt
    # $executable $i $nonlinear_error $on_cpu >> results_nonlinear_error_cpu.txt
    # $executable $i $nonlinear_error $on_gpu >> results_nonlinear_error_gpu.txt
done
