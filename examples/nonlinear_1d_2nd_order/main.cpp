...
int main(int argc, char* argv[]) {
    ...
    auto lhs = std::make_shared<NonlinearLHS>(gpu_threads, cpu_threads, eqn);
    ...
    auto iterator = std::make_shared<IteratorNonlinear>(gpu_threads, cpu_threads_2, eqn);
    // auto iterator = std::make_shared<IteratorAsync>(gpu_threads, cpu_threads_2, eqn);
    auto restrictor = std::make_shared<pmf::modules::RestrictorFullWeighting>(gpu_threads, cpu_threads_2);
    auto interpolator = std::make_shared<pmf::modules::InterpolatorLinear>(gpu_threads, cpu_threads_2);
    ...
    solver->set_cpu_threads(cpu_threads_2);
    ...
    if (boundary_conditions->is_west_neumann() && boundary_conditions->is_east_neumann()) {
        double a = 0;
    ...
    // dump_results(soln, *eqn, grid->get_x(), "results_vcycle_final.csv");
